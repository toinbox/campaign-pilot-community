...
TRANSLATIONS = {
    ...
    "form_tracking_domain_help": {"cs": "Doména pro tracking a odhlášení (např. https://example.com). Musí směřovat na tento server. Pokud prázdné, použije se APP_BASE_URL.", "en": "Domain for tracking & unsubscribe (e.g. https://example.com). Must point to this server. Falls back to APP_BASE_URL if empty.", "de": "Domain für Tracking & Abmeldung (z.B. https://example.com). Muss auf diesen Server zeigen. Fällt auf APP_BASE_URL zurück.", "ru": "Домен для трекинга и отписки (напр. https://example.com). Должен указывать на этот сервер. Если пусто, используется APP_BASE_URL.", "es": "Dominio para tracking y desuscripción (ej. https://example.com). Debe apuntar a este servidor. Si vacío, usa APP_BASE_URL."},
